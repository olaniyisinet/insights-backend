...
const getGraphData = ({input, user, tenant}) => {
    ...
    let regoCertificateUnit = ppaKpi.regoCertificateUnit;
    ...
    return {
        ...
        graphDataUnit: regoCertificateUnit,
        graphData: graphData
    };
};
...
const getGraphDataPoints = (input, graphRange, ppaKpi) => {
    let graphData = [];
    let regoActualMin = ppaKpi.regoActualMin;
    let regoActualMax = ppaKpi.regoActualMax;
    let regoExpectedMin = ppaKpi.regoExpectedMin;
    let regoExpectedMax = ppaKpi.regoExpectedMax;
    if(input.graphOption === 'YESTERDAY' 
        ...
        for (let slot = 0; slot < 24; slot++) {
            ...
            populateRandomDataPoint('Actual', graphData, {
                ...
                minNumber: regoActualMin,
                maxNumber: regoActualMax,
                isZero: isZeroSlotOne
            });
            populateRandomDataPoint('Expected', graphData, {
                timeSlot: slotOneName,
                multiplicationFactor: 1/48,
                minNumber: regoExpectedMin,
                maxNumber: regoExpectedMax,
                isZero: isZeroSlotOne
            });
        }
