...
const getGraphDataPoints = (input, cs) => {
    ...
    if(input.graphOption === 'TODAY' || input.graphOption === 'LASTDAY') {
        let assetvsGridPercentage = percentageMin;
        graphData.push({ type: 'Renewable Energy', value: assetvsGridPercentage });
        graphData.push({ type: 'Grid Energy', value: 100 - percentageMin });

    } else { 
        let assetvsGridPercentage = randomIntBetween(percentageMin, percentageMax);
        graphData.push({ type: 'Renewable Energy', value: assetvsGridPercentage });
        graphData.push({ type: 'Grid Energy', value: 100 - perceassetvsGridPercentagentageMin });
    }
