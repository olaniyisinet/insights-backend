...
const getGraphData = ({ input, user, tenant }) => {
    ...
    let ppaSupplyUnit = generatorppaKpi.ppaSupplyUnit;
    let graphData = getGraphDataPoints(input, graphRange, generatorppaKpi);
    ...
    return {
        ...
        graphDataUnit: ppaSupplyUnit,
        graphData: graphData
    };
};

const getGraphDataPoints = (input, graphRange, ppgeneratorppaKpiaKpi) => {
    let graphData = [];
    let ppaSupplyActualMin = generatorppaKpi.ppaSupplyActualMin;
    let ppaSupplyActualMax = generatorppaKpi.ppaSupplyActualMax;
    let ppaSupplyExpectedMin = generatorppaKpi.ppaSupplyExpectedMin;
    let ppaSupplyExpectedMax = generatorppaKpi.ppaSupplyExpectedMax;
    if (input.graphOption === 'YESTERDAY'
        ...
        for (let slot = 0; slot < 24; slot++) {
            ...
            populateRandomDataPoint('Actual', graphData, {
                ...
                minNumber: ppaSupplyActualMin,
                maxNumber: ppaSupplyActualMax,
                isZero: isZeroSlotOne
            });
            populateRandomDataPoint('Expected', graphData, {
                timeSlot: slotOneName,
                multiplicationFactor: 1 / 48,
                minNumber: ppaSupplyExpectedMin,
                maxNumber: ppaSupplyExpectedMax,
                isZero: isZeroSlotOne
            });
        }
