...
const getGraphData = ({input, user, tenant}) => {
    ...
    let cs = dfm.assetsPortfolio;
    let revenuePortfolioUnit = 'KWh';
...
const getGraphDataPoints = (input, cs) => {
    ...
    if (input.graphOption === 'LAST7DAYS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(100, 500) * 1);
          }
    } else if (input.graphOption === 'LAST30DAYS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(100, 500) * 5);
          }
    } else if (input.graphOption === 'LAST6MONTHS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(100, 500) * 15);
          }
    }
    else if (input.graphOption === 'LAST12MONTHS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(100, 500) * 30);
          }
    }
