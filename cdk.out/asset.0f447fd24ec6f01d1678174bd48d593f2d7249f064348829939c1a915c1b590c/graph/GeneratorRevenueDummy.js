...
const getGraphData = ({input, user, tenant}) => {
    ...
    let cs = dfm.assetsRevenue;
    let revenuePerDayUnit = cs.revenuePerDayUnit;
    let revenuePerDayMin = cs.revenuePerDayMin;
    let revenuePerDayMax = cs.revenuePerDayMax;
    let graphData = getGraphDataPoints(input, graphRange, revenuePerDayMin, revenuePerDayMax);
    ...
    return {
        ...
        graphDataUnit: revenuePerDayUnit,
        graphData: graphData
    };
};
...
const populateRandomDataPoint = (graphData, props) => {
    ...
    let revenue = randomFloatBetween(minNumber, maxNumber) * mf;
    let roundedRevenue = mf <1  ? parseFloat(revenue.toFixed(2)) : Math.floor(revenue);
...
const getGraphDateRange = (input) => {
    ...
    if(input.graphOption === 'YESTERDAY' || input.graphOption === 'LASTDAY') {
        ...
        intervalValue = '1';
        intervalUnit = 'months';
        graphStartTime.setFullYear(graphStartTime.getMonth() - 1);
        graphStartTime = setStartTimeSlot(graphStartTime);
