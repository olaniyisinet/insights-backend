...
const getGraphDataPoints = (input, cs) => {
    ...
    if (input.graphOption === 'LAST7DAYS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(100, 1000) * 1);
          }
    } else if (input.graphOption === 'LAST30DAYS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(1000, 3000) * 1);
          }
    } else if (input.graphOption === 'LAST6MONTH') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(2000, 6000) * 15);
          }
    }
    else if (input.graphOption === 'LAST12MONTHS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(5000, 10000) * 30);
          }
    }
