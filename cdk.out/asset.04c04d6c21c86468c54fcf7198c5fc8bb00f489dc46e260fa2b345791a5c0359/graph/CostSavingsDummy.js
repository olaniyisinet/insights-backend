...
const getGraphData = ({input, user, tenant}) => {
    ...
    let graphData = getGraphDataPoints(input, graphRange, costSavingsPerDayMin, costSavingsPerDayMax);
...
const populateRandomDataPoint = (graphData, props) => {
    ...
    let costSaving = randomFloatBetween(minNumber, maxNumber) * mf;
    let roundedSaving = mf <1  ? parseFloat(costSaving.toFixed(2)) : Math.floor(costSaving);

    let param = {
        name: 'Your Cost Savings',
        date: props.timeSlot
    };
    if(!props.isZero)
        param.saving = roundedSaving;
