...
const getGraphData = ({input, user, tenant}) => {
    ...
    const assetsRego = dfm.assetsREGO;
    let assetsRegoCertificateUnit = assetsRego.assetsRegoCertificateUnit;
    let graphData = getGraphDataPoints(input, graphRange, assetsRego);
    ...
    return {
        ...
        graphDataUnit: assetsRegoCertificateUnit,
        graphData: graphData
    };
};

const getGraphDataPoints = (input, graphRange, assetsRego) => {
    let graphData = [];
    let assetsRegoActualMin = assetsRego.assetsRegoActualMin;
    let assetsRegoActualMax = assetsRego.assetsRegoActualMax;
    let assetsRegoExpectedMin = assetsRego.assetsRegoExpectedMin;
    let assetsRegoExpectedMax = assetsRego.assetsRegoExpectedMax;
    if(input.graphOption === 'YESTERDAY' 
        ...
        for (let slot = 0; slot < 24; slot++) {
            ...
            populateRandomDataPoint('Actual', graphData, {
                ...
                minNumber: assetsRegoActualMin,
                maxNumber: assetsRegoActualMax,
                isZero: isZeroSlotOne
            });
            populateRandomDataPoint('Expected', graphData, {
                timeSlot: slotOneName,
                multiplicationFactor: 1/48,
                minNumber: assetsRegoExpectedMin,
                maxNumber: assetsRegoExpectedMax,
                isZero: isZeroSlotOne
            });
        }
