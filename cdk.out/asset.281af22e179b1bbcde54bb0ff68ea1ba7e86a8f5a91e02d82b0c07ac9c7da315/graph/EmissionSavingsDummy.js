...
const getGraphData = ({input, user, tenant}) => {
    ...
    let es = dfm.emissionSavings;
    let emissionUnit = es.emissionSavingsPerDayUnit;
    let emissionrDayMin = es.emissionSavingsPerDayMin;
    let emissionDayMax = es.emissionSavingsPerDayMax;
    let graphData = getGraphDataPoints(input, graphRange, emissionrDayMin, emissionDayMax);
    ...
    return {
        ...
        graphDataUnit: emissionUnit,
        graphData: graphData
    };
};

const getGraphDataPoints = (input, graphRange, minNumber, maxNumber) => {
    let graphData = [];
    if(input.graphOption === 'YESTERDAY' 
        ...
        for (let slot = 0; slot < 24; slot++) {
            ...
            populateRandomDataPoint( graphData, {
            ...
            populateRandomDataPoint( graphData, {
                timeSlot: slotTwoName,
                multiplicationFactor: 1/48,
                minNumber: minNumber,
                maxNumber: maxNumber,
                isZero: isZeroSlotTwo
            });
        }
...
const populateRandomDataPoint = (graphData, props) => {
    let mf = props.multiplicationFactor;
    let minNumber  =  props.minNumber;
    let maxNumber  =  props.maxNumber;
    let emissionSaving = randomFloatBetween(minNumber, maxNumber) * mf;
    let roundedSaving = mf <1  ? parseFloat(emissionSaving.toFixed(2)) : Math.floor(emissionSaving);

    let param = {
        name: 'CO2 Savings by You',
        date: props.timeSlot
    };
    if(!props.isZero)
        param.emission = roundedSaving;
