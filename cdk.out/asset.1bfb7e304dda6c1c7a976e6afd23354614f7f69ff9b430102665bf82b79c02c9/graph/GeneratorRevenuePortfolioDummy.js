...
const getGraphData = ({input, user, tenant}) => {
    ...
    let graphData = dfm.assetsRevenuePortfolio;//getGraphDataPoints(input, cs);
...
const getGraphDataPoints = (input, cs) => {
    ...
    if (input.graphOption === 'LAST7DAYS') {
        cs.value = Math.floor(Math.random() * 1000);
    } else if (input.graphOption === 'LAST30DAYS') {
        cs.value = Math.floor(Math.random() * 5000);
    } else if (input.graphOption === 'LAST6MONTH') {
        cs.value = Math.floor(Math.random() * 10000);
    }
    else if (input.graphOption === 'LAST12MONTHS') {
        cs.value = Math.floor(Math.random() * 150000);
    }
