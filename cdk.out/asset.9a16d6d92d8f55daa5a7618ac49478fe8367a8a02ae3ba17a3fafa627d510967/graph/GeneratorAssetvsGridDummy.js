...
const getGraphData = ({input, user, tenant}) => {
    ...
    let cs = dfm.assetvsGrid.data;
    let assetvsGridUnit = dfm.assetvsGridUnit;
...
const getGraphDataPoints = (input, cs) => {
    ...
    if (input.graphOption === 'LAST7DAYS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(500, 1000) * 1);
          }
    } else if (input.graphOption === 'LAST30DAYS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(1000, 4000) * 5);
          }
    } else if (input.graphOption === 'LAST6MONTHS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(4000, 6000) * 15);
          }
    }
    else if (input.graphOption === 'LAST12MONTHS') {
        for (const item of graphData) {
            item.value = Math.floor(randomFloatBetween(6000, 10000) * 30);
          }
    }
